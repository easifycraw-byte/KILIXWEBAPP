-- Compatibility hardening for in-app account deletion.
-- Re-applies the deletion contract so products owned through the user's stores
-- are removed from public.products before the stores themselves are deleted.
...
BEGIN
  ...
  SELECT COUNT(*)::integer INTO v_owned_stores FROM public.stores WHERE owner_id = p_uid;

  DELETE FROM public.orders WHERE user_id = p_uid;
  GET DIAGNOSTICS v_deleted_orders = ROW_COUNT;

  UPDATE public.orders o
  SET store_id = NULL, product_id = NULL
  WHERE o.store_id IN (SELECT s.id FROM public.stores s WHERE s.owner_id = p_uid);
  ...
  WHERE p.store_id = s.id AND s.owner_id = p_uid;
